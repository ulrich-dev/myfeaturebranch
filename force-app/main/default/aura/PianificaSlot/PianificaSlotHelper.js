({
    ...
    saveApex: function(component, event, helper, allSlots, trashSlotList){
        ...
        spinner.incrementCounter();
        //CR
        var listaModello = component.get("v.slotModello");
        //CR
		var action = component.get ('c.saveSlotsApex');
		action.setParams({
            upsertList: allSlots,
            deleteList: trashSlotList,
            slotModello: listaModello
		});
		...
		action.setCallback(this, function(response){
            ...
            if (state === "SUCCESS") {
                ...
                filialeNameList.forEach(filiale =>{
                    ...
                    mess[i] = 'Esiste almeno un attività nella zona '+filiale+' con  priorita alta ed una data di scadenza '+caseList[0].Cases[0].CreatedDate.substr(0, 10);
    ...
    isEmpty: function(obj) {
        ...
        //Modifica Modello
    caricaListaModelli : function(cmp, event){
        var spinner = cmp.find('spinnerComponent');
        var action=cmp.get("c.caricaModelliCTRL");
        action.setCallback(this,function(resp){
            spinner.decreaseCounter();
            if(resp.getState()=='SUCCESS'){
                cmp.set("v.modelliEsistenti",resp.getReturnValue());
            }
            else if(resp.getState()=='ERROR'){
                var errors = resp.getError();
                if (errors) {
                    if (errors[0] && errors[0].message) {
                        ...
                        console.log("Unknown error");
                        this.showToast('Errore generico','error');
                    }
                } else {
                    console.log("Unknown error");
                    this.showToast('Errore generico','error');
    ...
    caricaSlot : function(cmp, event){
        this.submitNewTargetDate(cmp, event, this, false);
        var idModello = cmp.get("v.selezioneModello");
       if(idModello == null || idModello == '') return;
        var spinner = cmp.find('spinnerComponent');
        var action=cmp.get('c.caricaSlotCTRL');
        action.setParams({
            ident: idModello
        });
        action.setCallback(this,function(resp){
            ...
            if(resp.getState()=='SUCCESS'){
				this.onchangeSettimana(cmp, event);
                var wrapper= resp.getReturnValue();
                cmp.set("v.initWrapperMod", wrapper);
                cmp.set("v.usersMod",wrapper.utenti);
                cmp.set("v.draftSlotListMod",wrapper.slotList);
                cmp.set("v.slotListMod",wrapper.slotList);
                cmp.set("v.todayMod",wrapper.oggi);
                cmp.set("v.weekDaysMod",wrapper.dataWrapperList);
                cmp.set("v.nomeMod", wrapper.nome);
                cmp.set("v.descrizioneMod", wrapper.descrizione);
    ...
    onchangeSettimana : function(cmp, event){
       var numSettimana = cmp.get("v.numSettimana");
       var dataPartenza = cmp.get("v.targhetDate");
      
        var spinner = cmp.find('spinnerComponent');
        var action=cmp.get('c.getSettimanaCTRL');
        action.setParams({
            numSettimana: numSettimana,
            dataPartenza: dataPartenza
        });
        action.setCallback(this,function(resp){
           
            if(resp.getState()=='SUCCESS'){
                var dataResp= resp.getReturnValue();
               
                  
                cmp.set("v.settimana", dataResp);
    ...
    checkData : function(cmp, event){
        var dataPartenza = cmp.get("v.targhetDate");
        
        var spinner = cmp.find('spinnerComponent');
        var action=cmp.get('c.dateIsValid');
        action.setParams({
            dataPartenza: dataPartenza
        });     
        action.setCallback(this,function(resp){
            ...
            if(resp.getState()=='SUCCESS'){
            	if(resp.getReturnValue())this.caricaSlot(cmp,event);
				else this.showToast('Data errata. La data pianificazione deve partire dalla settimana successiva', 'error');
            }
            else if(resp.getState()=='ERROR'){
                var errors = resp.getError();
                if (errors) {
    ...
    applicaModelloDefinitivamente : function(cmp, event){
    	var settimana = cmp.get("v.settimana");
        var calendario = cmp.get("v.draftSlotListMod");
        
        var spinner = cmp.find('spinnerComponent');
        var action=cmp.get('c.applicaModCTRL');
        action.setParams({
            settimana: settimana,
        ...
        action.setCallback(this,function(resp){
            ...
            if(resp.getState()=='SUCCESS'){
                if(resp.getReturnValue() == null || resp.getReturnValue() == undefined){
                    this.showToast("Pianificazione già presente: Non è possibile sovrascrivere la pianificazione già esistente", "error");
                    spinner.decreaseCounter();
                    return;
                }
                var lista = cmp.get("v.draftSlotList");
                lista = resp.getReturnValue();
                var draftSlotList= cmp.get("v.draftSlotList");
                var slotModello = cmp.get("v.slotModello");
                
                
                lista.forEach(function(temp) {
  					draftSlotList.push(temp);
                    slotModello.push(temp);
                });
                
				cmp.set("v.slotModello", slotModello);
                cmp.set("v.draftSlotList",draftSlotList);
                cmp.set("v.slotListMod", lista);
                 cmp.set("v.slotList", lista);
                var stampa= cmp.get("v.draftSlotList");
                console.log('----');
                console.log(stampa);
                cmp.set('v.unsavedAdd', lista.length);
                this.showToast("Modello Applicato: Premere 'Salva' per apportare le modifiche","success");
                 cmp.set("v.ImpFiliale", true);
            }
            else if(resp.getState()=='ERROR'){
                var errors = resp.getError();
                if (errors) {
